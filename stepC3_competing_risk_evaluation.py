...
def write_readme(
    ...
    lines = [
        ...
        "## 研究目的",
        "",
        "C3 补充 C2 的限制：C2 主要是原因特异性排序/模型比较，C3 面向竞争风险下的 5 年绝对风险评价。C3 将 `status=2` 的 care home 前死亡作为竞争事件，用原因特异性 Cox 重建 care-home 累计发生函数。",
        "",
        "## 数据来源",
        "",
        "- 只使用 `fully_synthetic_stepC_v1/`。",
        "- 可选读取 C2 保存的 predictor list。",
        "- 没有使用真实 SLAM 数据。",
        "- 没有使用 Step B 数据。",
        "- 没有使用原始 CSV、死亡 Excel、WMH Excel。",
        "",
        "## 结局定义",
        "",
        "- `status=1` 是 care home entry / institutionalisation。",
        "- `status=2` 是 death before care home competing event。",
        "- `death_after_carehome` 不是主结局竞争事件。",
        ...
        "## 指标",
        "",
        "- MAE/RMSE vs true synthetic 5-year risk。",
        "- naive Brier at 5 years。",
        "- observed 5-year AUC。",
        "- calibration slope/intercept。",
        "- calibration deciles。",
        "- cause-specific C-index 仅作为与 C2 连续的辅助指标。",
        "",
        "## 主要结果",
        "",
        f"- replicate-level rows: {n_rows}; failures: {n_fail}; full_run_passed: {full_pass}.",
        f"- S0/S1/S2 中，最佳非 oracle 模型分别是 `{best_model('S0_linear_PH_inst30')}`, `{best_model('S1_linear_PH_inst15')}`, `{best_model('S2_linear_PH_inst45')}`。",
        f"- S6 high-dimensional sparse MRI 中，最佳非 oracle 模型是 `{best_model('S6_highdim_sparseMRI_inst30')}`。",
        f"- S7 strong death competing 中，最佳非 oracle 模型是 `{best_model('S7_strong_death_competing_inst30')}`。{s7_competing_text}",
        f"- 是否有模型异常超过 oracle: {any_oracle_flag}.",
        f"- Fine-Gray 是否成功运行: {bool(finegray_status['finegray_ran'].iloc[0]) if not finegray_status.empty else False}; 原因/状态见 `tables/finegray_optional_status.csv`。",
        f"- C3 与 C2 的结论是否一致: {c2_consistency}",
        "",
        "## Best Model Table",
        "",
        ...
        "## 限制",
        "",
        "- 数据是全合成，用于方法学测试，不是临床推断。",
        "- Fine-Gray 如果没有成功运行，已在 optional status 中写明。",
        "- 朴素 Brier 不是 IPCW Brier。",
        "- C3 第一版重点是 5 年风险，不是完整时间动态风险。",
        "- Cox DGM 仍然使用 fallback predictor list，除非 exact DGM coefficients 已导出。",
        "",
        "## 下一步建议",
        "",
        "- 如果 C3 通过，准备给 Daniel/Ash 的汇报。",
        "- 可选 C4：实现 IPCW Brier / time-dependent AUC / 更正式的 Fine-Gray pipeline。",
        "- 可选改进 Step C generator：导出 exact DGM predictor coefficients。",
        "",
    ]
